...
//                        UserDefaults.standard.setValue("\(dicData["user_id"]!)", forKey: USER_ID)
                        ...
                        if "\(dicData["registration_completed"]!)" == "0"{//false
                            ...
                            let dicParameters = NSDictionary(objects: [userID,email!,domainKey,"ios","\(OFAUtils.getAppVersion())","\(OFAUtils.getDeviceID())"], forKeys: ["user_id" as NSCopying,"email" as NSCopying,"domain_key" as NSCopying,"platform" as NSCopying,"app_version" as NSCopying,"device" as NSCopying])
